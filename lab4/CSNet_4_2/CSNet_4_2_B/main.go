...
func main() {
	...
	for {
		...
		fmt.Println("B主机解析IP包:")
		printIPPacket(ipPacket)

		// 转发到C主机
		if ipToString(ipPacket.DestinationIP) == HostEIP {
			ethernetFrame = createEthernetFrame(stringToMACBytes(HostBMAC), stringToMACBytes(HostCMAC), ipPacket)
			fmt.Println("B主机转发消息到C主机:")
			printEthernetFrame(ethernetFrame)

			// 发送以太网帧到C主机
			sendFrameToHost(ethernetFrame, HostCIP)
		}
	}
}
