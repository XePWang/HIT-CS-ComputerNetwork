...
func main() {
	// 监听UDP端口
	udpAddr, err := net.ResolveUDPAddr("udp", HostDIP+":"+Port)
	if err != nil {
	...
	for {
		...
		fmt.Println("D主机解析IP包:")
		printIPPacket(ipPacket)

		// 转发到E主机
		if ipToString(ipPacket.DestinationIP) == HostEIP {
			ethernetFrame = createEthernetFrame(stringToMACBytes(HostDMAC), stringToMACBytes(HostEMAC), ipPacket)
			fmt.Println("D主机转发消息到E主机:")
			printEthernetFrame(ethernetFrame)

			// 发送以太网帧到E主机
			sendFrameToHost(ethernetFrame, HostEIP)
		}
	}
}
