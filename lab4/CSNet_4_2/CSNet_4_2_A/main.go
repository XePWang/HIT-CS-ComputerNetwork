...
func main() {
	...
	for {
		...
		// 创建IP包和以太网帧
		ipPacket := createIPPacket(stringToIPBytes(HostAIP), stringToIPBytes(HostEIP), []byte(input))
		ethernetFrame := createEthernetFrame(stringToMACBytes(HostAMAC), stringToMACBytes(HostBMAC), ipPacket)
		fmt.Println("A主机发送消息:")
		printEthernetFrame(ethernetFrame)

		// 发送以太网帧到B主机
		sendFrameToHost(ethernetFrame, HostBIP)
	}
}
